...
- [x] **sorcery.conf had no `contact=` mapping — agent answer legs dropped on origination** — fixed 2026-09-06, three layers landed together: `contact=realtime,ps_contacts` in `sorcery_realtime_agents.conf.sample`, the `ps_contacts` family in `extconfig_realtime_agents.conf.sample`, and the table itself in `sip_store.py` bootstrap (`_DDL_PS_CONTACTS` — no call-engine migration creates it; migrations `0002`/`0006` cover the other five `ps_*` tables only). Unmapped, contacts lived only in pod-local astdb → `pjsip show contacts` Avail but `realtime load ps_contacts` empty → originate fell back to the bare endpoint name as a URI ("Could not create dialog to invalid URI 'staff_<agent>'"). Confirmed live on Azure nonprod. **All three layers must ship in the same image**: sorcery mapping without family+table breaks REGISTER itself. Needs image rebuild+redeploy; the nonprod pod only carries a non-persistent sed hotfix until then. Prod runs the same image, so it has the same latent gap.
